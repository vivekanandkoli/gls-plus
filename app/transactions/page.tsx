...
export default function TransactionsPage() {
  ...
  const buildBaseQuery = useCallback(() => {
    ...
    if (debouncedQ.trim()) query = query.ilike("clients.name", `%${debouncedQ.trim()}%`);
    if (debouncedInvoiceQ.trim()) query = query.ilike("invoice_number", `%${debouncedInvoiceQ.trim()}%`);
    const minAmt = parseFloat(debouncedAmountMin);
    const maxAmt = parseFloat(debouncedAmountMax);
    if (!isNaN(minAmt)) query = (query as any).gte("amount_thb", minAmt);
    if (!isNaN(maxAmt)) query = (query as any).lte("amount_thb", maxAmt);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const { data, count, error } = await buildBaseQuery().range(start, end);
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      ...
      let ids: string[];
      if (selectAllPages) {
        // Fetch every matching ID (respects current filters, no pagination limit).
        let idsQuery = supabase.from("transactions").select("id");
        if (from) idsQuery = idsQuery.gte("date", from);
        if (to)   idsQuery = idsQuery.lte("date", to);
        if (type !== "ALL") idsQuery = idsQuery.eq("type", type);
        const { data: allRows, error: fetchErr } = await idsQuery;
        if (fetchErr) throw fetchErr;
        ids = ((allRows ?? []) as any[]).map((r) => r.id as string);
      } else {
        ids = [...selectedIds];
      }
  ...
  }, [selectedIds, selectAllPages, from, to, type, refresh, refreshStock]);
  ...
  const onExport = useCallback(async () => {
    ...
    try {
      ...
      let offset = 0;
      while (true) {
        const { data, error } = await (buildBaseQuery() as any)
          .range(offset, offset + batchSize - 1)
          .select("id,date,client:clients(name),type,invoice_number,weight_grams,rate_per_gram,amount_thb,vat_percent,notes");
        if (error) throw error;
